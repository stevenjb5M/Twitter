...
Why Refactor Tweeter to MVP (5:17)
An Explanation of the Code for the UserItemScroller component (11:49)
An M2A Refactoring Demo (1:39:55)
...
Each presenter class should define a view interface that is used by its component or hook. This is the method interface through which the presenter will call its view. For example, the view interface should include methods for things such as passing data to the view for display, displaying messages to the user, and enabling/disabling UI controls. See the demo video for examples of what this looks like.
...
Logic for managing paging (e.g., whether there are more pages, what the last retrieved item was, etc.),
Code that calls the service classes
